...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awreason
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awreason-*.whl dist/*.tar.gz ...]
...
_KEYSTONES = ("awreason/__init__.py", "awreason/cli.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awreason-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awreason/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awreason/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awreason/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awreason/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awreason/y.py": "from awreason.client import call\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awreason-0.0.0.tar.gz"
